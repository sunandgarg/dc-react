...
try {
  ...
  if (result.skipped && result.message === "Daily post cap reached") {
    ...
    assert.match(studio.analysis_model_used, /^openai:gpt-5\.6-luna$/, "Blog Studio analysis did not use OpenAI GPT-5.6 Luna");
    assert.match(studio.writing_model_used, /^openai:gpt-5\.6-luna$/, "Blog Studio writing did not use OpenAI GPT-5.6 Luna");
  ...
  console.log(JSON.stringify({
    ok: true,
    openai_blog: `${verificationMode} verified with GPT-5.6 Luna analysis and writing`,
