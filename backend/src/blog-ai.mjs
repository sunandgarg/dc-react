...
// Keep the contract focused on reader intent. Requiring the same "Key facts /
// Decision guidance / FAQs" headings on every page was producing the exact
// symmetrical outline we are trying to prevent. FAQs remain validated below,
// but do not need to be a forced heading in every article.
const DEFAULT_REQUIRED_SECTIONS = ["Answer first"];
...
export const DEKHOCAMPUS_HUMAN_EDITORIAL_POLICY = Object.freeze({
  ...
  formatting: "No raw Markdown syntax. The application receives semantic HTML, so use real headings, paragraphs and useful lists. Use a table only when it genuinely improves a long-form comparison; compact 350-400 word articles must use labelled lists or concise prose instead of a table. FAQs are stored and rendered in their own dedicated section.",
  ...
  semantic_structure: "Build a clean semantic HTML skeleton for search systems: use H2/H3 headings for real questions or decisions, put a concise direct answer in the first paragraph under each main H2 when the section is prose-led, and use UL/OL lists for genuine steps, statistics or features. Keep the HTML accessible and do not emit Markdown.",
  ...
  banned: [
    ...
    "crucial", "robust", "pivotal", "multifaceted", "seamless", "bespoke", "groundbreaking", "imperative", "paramount", "overarching", "dynamic", "nuanced", "quintessential", "transformative",
...
const GENERIC_REQUIRED_SECTION_NAMES = new Set(["key facts", "decision guidance", "faqs", "frequently asked questions"]);
...
export function normalizeBlogAgentSettings(value = {}) {
  ...
  const configuredSections = normalizeStringList(value.required_sections, [], 8)
    .filter((section) => !GENERIC_REQUIRED_SECTION_NAMES.has(normalizeArticleTitle(section)));
  return {
    ...value,
...
const FLATTENED_TABLE_COPY_PATTERNS = [
  /\bstudent\s+profile\b[\s\S]{0,220}\btarget\s+courses\b[\s\S]{0,220}\brisk\s+to\s+check\b[\s\S]{0,220}\bsafer\s+approach\b/i,
  /\bcheck\b[\s\n:|]+what\s+to\s+confirm[\s\n:|]+real\s+consequence[\s\n:|]+recognition\b/i,
  /\bitem\b[\s\n:|]+use\s+it\s+for[\s\n:|]+check\s+this\s+risk[\s\n:|]+city\s+information\b/i,
];
const FLATTENED_TABLE_LABEL_PATTERN = /\b(?:student\s+profile|target\s+courses|risk\s+to\s+check|safer\s+approach|target\s+course\s+type|check\s+first|possible\s+result\s+if\s+missed|safer\s+decision)\b/gi;
const GENERIC_VERIFICATION_PATTERN = /\b(?:check|verify|recheck|read|confirm)\b[\s\S]{0,65}\b(?:official|authority|notification|portal|notice)\b/gi;
...
function containsFlattenedTableCopy(body, contentHtml = "") {
  // A real semantic table is allowed to use the same labels. Only flag the
  // labels when the article has no table markup to carry their structure.
  ...
  return labels.size >= 3 || FLATTENED_TABLE_COPY_PATTERNS.some((pattern) => pattern.test(String(body || "")));
}
...
function hasFormulaicSectionSequence(headings) {
  ...
  for (const heading of headings) {
    if (heading === FORMULAIC_SECTION_SEQUENCE[cursor] || heading.startsWith(`${FORMULAIC_SECTION_SEQUENCE[cursor]} `)) cursor += 1;
...
export function indiaDayWindow(reference = new Date()) {
  const indiaOffsetMs = 330 * 60_000;
  const shifted = new Date(reference.getTime() + indiaOffsetMs);
  const startMs = Date.UTC(shifted.getUTCFullYear(), shifted.getUTCMonth(), shifted.getUTCDate()) - indiaOffsetMs;
...
export async function loadArticleCoverage(siteScope = "dekhocampus", client = prisma, coverageWindow = null) {
  const createdAt = coverageWindow?.start || coverageWindow?.end
    ? {
      ...(coverageWindow.start ? { gte: coverageWindow.start } : {}),
      ...(coverageWindow.end ? { lt: coverageWindow.end } : {}),
    }
    : undefined;
  return client.articles.findMany({
    where: { site_scope: normalizeArticleSiteScope(siteScope), ...(createdAt ? { created_at: createdAt } : {}) },
...
export async function assertArticleTopicsAvailable(candidates, {
  ...
  for (const [candidateScope, scopedCandidates] of candidatesByScope) {
    const coverage = (await loadArticleCoverage(candidateScope, client, coverageWindow)).filter((article) => !excluded.has(String(article.id)));
...
export async function ensureSupportedAiModels() {
  ...
  await Promise.all([
    ...
    prisma.ai_providers.updateMany({
      where: { provider_name: "openai", default_model: { in: legacyOpenAiBlogModels } },
      data: { default_model: DEFAULT_OPENAI_TEXT_MODEL, updated_at: new Date() },
    }).catch(() => null),
    prisma.ai_runtime_controls.updateMany({
      where: { feature: { in: ["blog-studio", "blog-agent"] }, provider: "openai", model: { in: legacyOpenAiBlogModels } },
      data: { model: DEFAULT_OPENAI_TEXT_MODEL, updated_at: new Date() },
    }).catch(() => null),
    prisma.blog_ai_provider_settings.updateMany({
      where: { text_model: { in: legacyOpenAiBlogModels } },
      data: { text_model: DEFAULT_OPENAI_TEXT_MODEL, updated_at: new Date() },
    }).catch(() => null),
    prisma.blog_auto_agent_settings.updateMany({
      where: { text_model: { in: legacyOpenAiBlogModels } },
      data: { model_provider: "openai", text_model: DEFAULT_OPENAI_TEXT_MODEL, updated_at: new Date() },
    }).catch(() => null),
  ]);
}
...
async function researchSignals(limit = MAX_RESEARCH_SOURCES, siteScope = "dekhocampus") {
  ...
  const defaults = normalizedScope === "sarkari"
    ...
    : [
      { name: "Sarvgyan Education Articles", url: "https://www.sarvgyan.com/category/articles", source_type: "competitor_gap" },
      { name: "Shiksha Education News", url: "https://www.shiksha.com/news", source_type: "competitor_gap" },
...
export function articlePrompt(topic, signals, wordLimit = 0, correctionIssues = [], rawEditorialSettings = {}, requestedSiteScope = "dekhocampus", batchSiblings = []) {
  ...
  const primaryKeyword = String(typeof topic === "string"
    ? topic
    : topic?.primary_keyword || topic?.primary_entity || topicTitle).trim();
  const secondaryKeywords = normalizeStringList(
    typeof topic === "object" && topic !== null
      ? (Array.isArray(topic.secondary_keywords) ? topic.secondary_keywords : topic.tags)
      : [],
    [],
    10,
  );
  ...
  const topicBrief = typeof topic === "string"
    ...
    : {
      ...
      impact_reason: String(topic?.impact_reason || ""),
      time_signal: String(topic?.time_signal || ""),
...
Editorial contract:
- Role: act as a veteran niche education journalist and senior SEO content strategist. Produce authoritative, deeply engaging, human-sounding work that satisfies the stated intent and demonstrates E-E-A-T without claiming experience the evidence cannot support.
- Article specification: primary keyword "${primaryKeyword}"; secondary keywords ${JSON.stringify(secondaryKeywords)}; target audience "${editorial.audience}"; search intent "${searchIntent}"; target length about ${targetWords} words.
...
- Target about ${targetWords} words, using only the length the topic genuinely needs. When the target is 350 or 400, this is compact mode: keep the main content_html body between 350 and 400 words, excluding the separate faqs array. Compact mode is list-led: do not output an HTML table, table-like rows, or a sequence of labels; use a short explanatory paragraph followed by a real <ul> or <ol> when a comparison or checklist is needed.
...
- Follow the formatting blackout for source text: no raw Markdown headings, fences, bullet markers, numbered markers, Markdown tables, bold or italic markers. Return semantic HTML only because the site renderer and accessibility layer require headings, lists and (for long-form articles) tables. In compact mode, use headings, paragraphs and lists only. FAQs are stored and rendered separately.
...
- Never begin the article with prompt residue such as "Answer first:", "Answer:", "Executive summary:" or "Here is the answer:". Start with a natural, topic-specific sentence that gives the reader the useful consequence immediately, without announcing the writing instruction.
- Do not force a symmetrical counsellor template such as Executive summary -> Key facts -> Conceptual rationale -> Step-by-step guide -> Risk matrix -> Red flags -> FAQs. Choose a topic-native outline; vary section order, heading wording and section length according to the actual decision a reader faces.
- Never paste a comparison matrix as a flattened sequence of labels in prose. If a table genuinely clarifies a long-form choice, output a semantic HTML table with a thead, labelled th cells, tbody rows and real td values. For compact 350-400 word articles, never output a table: use a labelled bullet list or short descriptive paragraphs instead of rows copied line by line.
- Avoid one uniform didactic voice. Let the article contain measured judgement, a practical observation, a short aside or a direct reader question when the evidence supports it. Keep the advice trustworthy without making every paragraph sound like the same counsellor script.
- Consolidate repeated cautions. State the official-verification rule once, then apply it to the specific date, subject, fee or eligibility consequence instead of repeating the same hedge under several headings. A reader should not see the same "check the official notice" paragraph three different ways.
- Add grounded specifics when the private evidence supports them: a named authority, institution, subject code, paper section, counselling portal, date, fee rule or realistic student scenario. For subject-choice articles, name the actual universities or exam bodies in the evidence and explain one real mismatch risk. Never invent a gritty detail merely to make prose sound human.
...
- Answer the search intent directly in the first 2-3 sentences. Do not begin with a definition, scene-setting filler, a label such as "Answer first:", or phrases such as "In today's fast-paced digital world".
...
- Use descriptive H2/H3 headings and bullets where useful. Long-form articles may include one genuine comparison or summary table with labelled headers when a table improves the decision; compact 350-400 word articles must use a semantic list instead. Never turn table headings and cells into a plain-text stack.
...
export function assessGeneratedArticle(draft, topic, wordLimit = 0, rawEditorialSettings = {}) {
  ...
  const openingParagraph = contentHtml.match(/<p\b[^>]*>([\s\S]*?)<\/p>/i)?.[1];
  const openingText = stripHtml(openingParagraph || body).trim();
  const hasPromptResidueOpening = PROMPT_RESIDUE_OPENING_PATTERN.test(openingText);
  ...
  const hasDecisionStructure = compactWordMode
    ? /<(?:ul|ol)\b/i.test(contentHtml)
    : /<table\b/i.test(contentHtml) && /<th\b/i.test(contentHtml);
  check("Decision structure", hasDecisionStructure, 3, compactWordMode
    ? "compact articles need a semantic list for comparisons or checklists; do not flatten rows into prose"
    : "article needs at least one comparison or summary table with labelled headers", true);
...
Reject raw Markdown syntax, including hash headings, fenced code, Markdown bullets, numbered markers, pipe tables and bold or italic markers. Reject a flattened label stack even when it is wrapped in paragraphs. In compact mode, prefer a real semantic list over any table. Require a genuinely human cadence with at least one short punchy sentence and one longer explanatory sentence, but do not reward spelling mistakes or fake slang. Apply the full DekhoCampus banned lexicon from the human editorial policy, not only the short legacy list.
...
async function generateDraft(topic, { wordLimit = 0, cover = {}, signals = null, requiredTitle = "", editorialSettings = {}, model: requestedModel = "", feature = "blog-studio", siteScope = "dekhocampus", batchSiblings = [] } = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const prompt = attempt > 0 && draft && correctionIssues.length
      ? `${articleRevisionPrompt(draft, topic, evidence, correctionIssues, editorial, normalizedScope, batchSiblings)}${isCompactArticleWordTarget(targetWords) ? "\n\nCompact mode rule: keep the main content_html body between 350 and 400 words, excluding the separate faqs array. Use semantic headings, paragraphs and lists only; remove every table and any flattened label stack." : ""}`
...
export async function handleBlogAiSettings(request, userId) {
  ...
  await Promise.all([
    prisma.blog_auto_agent_settings.updateMany({
      data: { model_provider: blogTextProvider(updates.text_model), text_model: updates.text_model, updated_at: new Date() },
    }),
    prisma.ai_runtime_controls.updateMany({
      where: { feature: { in: ["blog-studio", "blog-agent"] } },
      data: { provider: blogTextProvider(updates.text_model), model: updates.text_model, updated_at: new Date(), updated_by: userId },
    }),
  ]);
...
function normalizeStudioDraft(value = {}, defaultCategory = "Education") {
  return {
    ...
    source_logo: String(toStoredMediaKeys(value.source_logo || "")).trim(),
...
async function uniqueAutoArticleSlug(tx, siteScope, requestedSlug, reference = new Date()) {
  const base = slugify(requestedSlug) || `article-${reference.getTime()}`;
  const first = await tx.articles.findFirst({ where: { site_scope: siteScope, slug: base }, select: { id: true } });
  ...
  let suffix = 1;
  while (suffix < 100) {
    const candidate = `${base}-${dateStamp}${suffix === 1 ? "" : `-${suffix}`}`;
    const existing = await tx.articles.findFirst({ where: { site_scope: siteScope, slug: candidate }, select: { id: true } });
...
export async function runBlogAgent(body = {}) {
  ...
  const dayWindow = indiaDayWindow();
  const todayCount = await prisma.$queryRawUnsafe("SELECT COUNT(*) AS total FROM `articles` WHERE `site_scope` = 'dekhocampus' AND `created_at` >= ? AND `created_at` < ? AND JSON_CONTAINS(`tags`, JSON_QUOTE('auto-blog-agent'))", dayWindow.start, dayWindow.end);
  ...
  const todayTrendCount = !entityContext && settings.google_trends_daily_enabled
    ? await prisma.$queryRawUnsafe("SELECT COUNT(*) AS total FROM `articles` WHERE `site_scope` = 'dekhocampus' AND `created_at` >= ? AND `created_at` < ? AND JSON_CONTAINS(`tags`, JSON_QUOTE('google-trends-daily'))", dayWindow.start, dayWindow.end)
    : [{ total: 0 }];
  ...
  try {
    ...
    const allCoverage = await loadArticleCoverage("dekhocampus");
    const todayCoverage = await loadArticleCoverage("dekhocampus", prisma, dayWindow);
    ...
    // Historical coverage guides the angle. Same-day coverage is the hard gate,
    // so a later verified update can exist without allowing headline churn today.
    const promptCoverage = [...new Set(allCoverage.map(compactArticleCoverage).filter(Boolean))]
      .slice(0, MAX_TOPIC_PROMPT_FINGERPRINTS);
    const topics = [];
    const comparedCoverage = [...todayCoverage];
    const rejected = [];
            ...
            topics: {
              ...
              items: {
                ...
                properties: {
                  ...
                  time_signal: { type: "STRING" },
                  category: { type: "STRING" },
                  tags: { type: "ARRAY", items: { type: "STRING" } },
                  trend_based: { type: "BOOLEAN" },
                },
                required: ["title", "angle", "search_intent", "primary_entity", "unique_value", "impact_reason", "time_signal", "trend_based"],
              },
            },
          },
          required: ["topics"],
