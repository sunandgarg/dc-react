...
const normalizeTextModel = (value?: string) => {
  ...
  return !model || model === "gpt-5.6-sol" ? DEFAULT_TEXT_MODEL : model;
};
    ...
    <div className="mb-4 rounded-2xl border bg-card p-4 shadow-sm">
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-3">
        ...
        <div>
          ...
          <p className="mt-1 text-[10px] text-muted-foreground">GPT-5.6 Luna handles research analysis and quality review and is the default writer. Other writing models remain selectable.</p>
          {supportsAdvancedSettings && (
            <select
            ...
            >
              <option value="gpt-5.6-luna">OpenAI GPT-5.6 Luna - default writer</option>
              <option value="gpt-6-sol">OpenAI GPT-6 Sol</option>
        ...
        <div>
          ...
          <div className="mt-1 flex flex-wrap gap-2">
            {[0, 350, 400, 500, 900, 1200, 1500, 1800].map(limit => <Button key={limit} size="sm" variant={settings.word_limit === limit ? "default" : "outline"} onClick={() => updateSetting("word_limit", limit)}>{limit === 0 ? "Adaptive" : limit === 350 ? "350-400 words" : `${limit} words`}</Button>)}
          </div>
          <p className="mt-1 text-[10px] text-muted-foreground">Adaptive chooses length from search intent. Choose 400 for a compact article or 500 for a slightly fuller version; FAQs stay in their separate FAQ section.</p>
        </div>
